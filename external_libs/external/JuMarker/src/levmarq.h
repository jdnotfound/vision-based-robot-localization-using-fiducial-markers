...
//}
template <typename T> class LevMarq
{
  public:
     ...
     * @param tau parameter indicating how near the initial solution is estimated to be to the real one. If 1, it means that it is
...
template <typename T> LevMarq<T>::LevMarq()
{
    _maxIters = 1000;
    _minErrorAllowed = 0;
    _der_epsilon = 1e-3;
...
template <typename T> bool LevMarq<T>::step(F_z_x f_z_x, F_z_J f_J)
{
...
template <typename T> double LevMarq<T>::getCurrentSolution(eVector &z)
{

    z = curr_z;
    return currErr;
}
template <typename T> double LevMarq<T>::solve(eVector &z, F_z_x f_z_x, F_z_J f_J)
{
