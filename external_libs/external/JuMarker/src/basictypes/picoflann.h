 ...
 * You only must define an adapter, that tells the class how to access the i-th dimension of your element. Here are two examples
showing how easy it is:
 *
 * You can use any container providing the methods size() ant at().
 * You must create the adapter that must implement the method :  T operator( )(const E &elem, int dim)const; T=float or double, E
is your element type
 *
 * You can easily save/read the index to/from an stream using toStream() and fromStream(). Please notice that the data of the
container is not copied in the KdTreeIndex so you
...
struct L2
{
    ...
    double compute_distance(const ElementType &elema, const ElementType &elemb, const Adapter &adapter, int ndims,
...
template <int DIMS, typename Adapter, typename DistanceType = L2> class KdTreeIndex
{
  ...
  public:
    ...
    inline std::vector<std::pair<uint32_t, double>> searchKnn(const Container &container, const Type &val, int nn,
                                                              bool sorted = true)
    {
    ...
    template <typename Type, typename Container>
    inline std::vector<std::pair<uint32_t, double>> radiusSearch(const Container &container, const Type &val, double dist,
    ...
    template <typename Type, typename Container>
    inline void radiusSearch(std::vector<std::pair<uint32_t, double>> &res, const Container &container, const Type &val,
  ...
  private:
    ...
    template <typename Container> void computeBoundingBox(BoundingBox &bbox, int start, int end, const Container &container)
    {
        bbox.resize(DIMS);
        for (int i = 0; i < DIMS; ++i)
    ...
    template <typename Type, typename Container>
    inline void generalSearch(std::vector<std::pair<uint32_t, double>> &res, const Container &container, const Type &val,
                              double dist, bool sorted = true, uint32_t maxNn = std::numeric_limits<int>::max()) const
    {
        double dists[DIMS];
        ...
        if (sorted && res.size() > 1)
            std::sort(res.begin(), res.end(), [](const std::pair<uint32_t, double> &a, const std::pair<uint32_t, double> &b) {
    ...
    class ResultSet
    {
      ...
      public:
        ResultSet(std::vector<std::pair<uint32_t, double>> &data_ref, uint32_t MaxSize = std::numeric_limits<uint32_t>::max(),
                  double MaxV = -1)
            : array(data_ref)
        {
            maxSize = MaxSize;
    ...
    template <typename Type, typename Container>
    inline void searchExactLevel(const Index &index, int64_t nodeIdx, const Type &elem, ResultSet &res, double mindistsq,
                                 double dists[], double epsError, const Container &container) const
    {
...
template <int DIMS, typename AAdapter, typename DistanceType> uint64_t KdTreeIndex<DIMS, AAdapter, DistanceType>::getHash() const
{
