...
class DirReader
{
  public:
    ...
    static std::vector<std::string> read(std::string path, std::string ext = "", const Params &params = Params())
    {
        ...
        if ((dir = opendir(path.c_str())) != NULL)
        {
            ...
            while ((ent = readdir(dir)) != NULL)
            {
                ...
                if (addit)
                {
                    if (params.setFullPath)
                        fname = path + getSeparator() + fname;
                    res.push_back(fname);
                }
            }
            closedir(dir);
    ...
    static std::string basename(std::string const &pathname)
    {
        ...
        return std::string(std::find_if(pathname.rbegin(), pathname.rend(), [](char c) { return c == getSeparator(); }).base(),
                           pathname.end());
    }
