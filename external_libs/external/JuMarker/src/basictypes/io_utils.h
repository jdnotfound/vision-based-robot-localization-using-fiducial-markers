...
template <typename T> inline T io_read(std::istream &str)
{
    T aux;
    str.read((char *)&aux, sizeof(T));
    return aux;
};

template <typename T> inline void io_write(const T &v, std::ostream &str)
{
    str.write((char *)&v, sizeof(T));
};

template <typename T> inline void io_read(T &v, std::istream &str)
{
    str.read((char *)&v, sizeof(T));
};
// use for simple elements
template <typename T> void toStream__(const std::vector<T> &v, std::ostream &str)
{
    uint32_t s = v.size();
...
template <typename T> void toStream__(const std::set<T> &v, std::ostream &str)
{
...
template <typename T> void fromStream__(std::vector<T> &v, std::istream &str)
{
...
template <typename T> void fromStream__(std::set<T> &v, std::istream &str)
{
...
template <typename key, typename value> void toStream__kv(const std::map<key, value> &t, std::ostream &str)
{
    uint32_t s = t.size();
...
template <typename Key, typename Value> void fromStream__kv(std::map<Key, Value> &t, std::istream &str)
{
    // now, the map
    uint32_t s;
    str.read((char *)&s, sizeof(s));
...
template <typename Tfirst_second> void toStream__kv_complex(const Tfirst_second &t, std::ostream &str)
{
    uint32_t s = t.size();
    str.write((char *)&s, sizeof(uint32_t));
    for (auto &x : t)
...
template <typename T> void toStream__ts(const std::vector<T> &v, std::ostream &str)
{
    uint32_t s = v.size();
    str.write((char *)&s, sizeof(s));
...
template <typename T> void fromStream__ts(std::vector<T> &v, std::istream &str)
{
    uint32_t s;
    str.read((char *)&s, sizeof(s));
    v.resize(s);
...
template <typename key> void toStream__kv(const std::map<key, cv::Mat> &t, std::ostream &str)
{
    uint32_t s = t.size();
    str.write((char *)&s, sizeof(uint32_t));
    for (auto &x : t)
...
template <typename Key> void fromStream__kv(std::map<Key, cv::Mat> &t, std::istream &str)
{
    // now, the map
    uint32_t s;
    str.read((char *)&s, sizeof(s));
