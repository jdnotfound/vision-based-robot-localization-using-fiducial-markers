...
TEST_NAME = "chilitag_dist_2.5m_angle_0deg"
...
print("====================================")
print("CHILITAGS DISTANCE + ACCURACY TEST")
print("====================================")
...
while total_frames < MAX_FRAMES:
    ...
    if len(detections) > 0:
        chosen_detection = detections[0]

        if EXPECTED_ID is not None:
            matching_detections = [
                det for det in detections
                if int(det["id"]) == EXPECTED_ID
            ]

            if len(matching_detections) > 0:
                chosen_detection = matching_detections[0]
            else:
                wrong_id_frames += 1

        selected_detection = chosen_detection
        marker_id_value = int(selected_detection["id"])
        ...
        if success:
            ...
            print(
                ...
                f"Z={tvec_z * 100:.1f} cm, "
                f"Dist={estimated_distance_m * 100:.1f} cm, "
            ...
            cv2.polylines(
                frame,
                [marker_2d_corners.astype(np.int32)],
                True,
                (0, 255, 255),
                2
            )
    ...
    cv2.imshow("Chilitags Distance Accuracy Metrics", frame)
